...
data class Match(
    ...
    companion object : ModelGenerator, QueryAble {
         ...
         */
        override fun query(apiConnection: ApiConnection): MatchQuery {
            return MatchQuery(apiConnection)
        }
    }
}
...
class MatchQuery(
    apiConnection: ApiConnection
        ...
        apiConnection,
        "match",
        { json: JSONObject -> Match.fromJson(json) },
        listOf("id", "matchday", "team_id")
) {
     ...
     */
    override fun query(): List<Match> {
        val models = super.query()
        val converted = mutableListOf<Match>()
        for (model in models) {
            converted.add(model as Match)
        }
        return converted
    }
}
